...
The bridge asks the API to claim the next job when something suggests there is one — on connect, when the API notifies it (POST /dispatch-notify, shared-secret gated, sent on approve/reconcile), when a job finishes, or when its once-a-minute presence write reports an approved job waiting (the fallback for a missed notification). It does not poll on a fixed interval. The API atomically claims at most one approved job before returning code; a notification only changes when the bridge asks, never what can be claimed. Presence is written to the database on connect, on disconnect and every 60s; the 10s WebSocket ping is local. Every 10 minutes the bridge logs a `bridge_api_calls` line with its API calls by action. The Pi receives job.run and reports job.accepted, job.output and job.exit. Updates are accepted only for jobs dispatched on that authenticated connection and are scoped to its robot ID in the API. The bridge serializes lifecycle messages to preserve their order.
