...
function seatRow(seatNumber, student){
  if (student){
    return `
      <div class="set-row">
        <span class="set-row-text"><span class="set-row-label">Seat ${seatNumber} · ${escapeHtml(student.email)}</span></span>
...
async function render(container, ctx){
  ...
  container.querySelectorAll('[data-add-student]').forEach((form) => {
    form.addEventListener('submit', async (e) => {
      ...
      try { await postAction({ action: 'add_student', email }); await render(container, ctx); }
  ...
  container.querySelectorAll('[data-add-admin-email]').forEach((form) => {
    form.addEventListener('submit', async (e) => {
      ...
      try { await postAction({ action: 'set_admin_email', slot, email }); await render(container, ctx); }
      catch (err){ showError(err); }
    });
  });
  ...
  container.querySelectorAll('[data-change-admin-email]').forEach((btn) => {
    btn.addEventListener('click', async () => {
      ...
      if (!email) return;
      try { await postAction({ action: 'set_admin_email', slot: Number(btn.dataset.slot), email: email.trim() }); await render(container, ctx); }
      catch (err){ showError(err); }
    });
  });
  ...
  container.querySelectorAll('[data-archive]').forEach((btn) => {
    btn.addEventListener('click', async () => {
      ...
      try { await postAction({ action: 'archive_student', studentId: Number(btn.dataset.archive) }); await render(container, ctx); }
      catch (err){ showError(err); }
    });
  });
  ...
  container.querySelectorAll('[data-restore]').forEach((btn) => {
    btn.addEventListener('click', async () => {
      try { await postAction({ action: 'add_student', email: btn.dataset.restore }); await render(container, ctx); }
      catch (err){ showError(err); }
    });
  });
  ...
  container.querySelectorAll('[data-delete]').forEach((btn) => {
    btn.addEventListener('click', async () => {
      ...
      try { await postAction({ action: 'delete_student', studentId: Number(btn.dataset.delete) }); await render(container, ctx); }
      catch (err){ showError(err); }
    });
  });
}
