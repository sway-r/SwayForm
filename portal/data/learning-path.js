          ...
          activities: [
            {
              ...
              summary: 'Watch SwayForm say hello — then change one number to make it wave five times.',
              workspaceFile: 'swayform_ws/src/swayform_robot/swayform_robot/behaviors/wave.py',
              relatedConcepts: ['Sequences', 'Repetition'],
              steps: [
                {
                  id: 'what-it-does', title: 'What It Does',
                  blocks: [
                    { type: 'lead', text: 'Wave is SwayForm saying hi. It lifts its arm, opens its hand, and swings its forearm back and forth — and while it does, the fingers ripple one after another, the way yours do when you wave to someone across a room. It\'s the first thing most people ask a robot to do, and it\'s the first thing you\'ll run.' },
                    { type: 'p', text: 'Nothing here is animation or a video. When you send this to the robot, a real arm moves, real fingers ripple, and it\'s the exact same instructions SwayForm uses every day — you\'re about to read them.' },
                  ],
                },
                {
                  id: 'what-to-watch', title: 'What to Watch',
                  blocks: [
                    { type: 'p', text: 'The whole wave takes a few seconds. Here\'s what to look for, in order:' },
                    { type: 'list', items: [
                    ...
                    { type: 'callout', tone: 'note', label: 'Look closely', text: 'The fingers don\'t all move at once. Each one starts a moment after the last, so the motion rolls across the hand. That small detail is what makes it look like a wave instead of a robot flapping.' },
                    { type: 'callout', tone: 'safety', label: 'Safety', text: 'Keep your hands clear of the arm while it\'s moving. It\'s gentle, but it doesn\'t know you\'re there.' },
                  ],
                },
                {
                  id: 'how-it-works', title: 'How It Works',
                  blocks: [
                    { type: 'p', text: 'A robot can\'t "just wave." It has to be told every step, in order — and a wave is really just a short list of steps:' },
                    { type: 'code', lang: 'text', filename: 'The recipe for a wave', code: 'go home\n  ↓\nopen hand\n  ↓\nlift arm into position\n  ↓\nswing forearm back and forth (fingers rippling)\n  ↓\ngo home' },
                    { type: 'p', text: 'That\'s it. Every gesture SwayForm makes — waving, shaking hands, fist-bumping — is a list like this one. The robot reads it top to bottom and does each step. When you learn to change these lists, you\'re learning to choreograph a robot.' },
                    { type: 'callout', tone: 'note', label: '"Home"', text: 'Home is the robot\'s neutral resting pose. Almost every gesture starts and ends there, so the robot always knows where it is before it begins the next thing.' },
                  ],
                },
                {
          ...
          activities: [
            {
              ...
              summary: 'Watch SwayForm offer a handshake — then decide how many times it should shake.',
              workspaceFile: 'swayform_ws/src/swayform_robot/swayform_robot/behaviors/handshake.py',
              relatedConcepts: ['Sequences', 'Timing'],
              steps: [
                {
                  id: 'what-it-does', title: 'What It Does',
                  blocks: [
                ...
                },
                {
                  ...
                  blocks: [
                    ...
                    { type: 'callout', tone: 'safety', label: 'Safety', text: 'You can put your hand in its hand — the grip is light. But never pull or push the arm while it\'s moving; let it do the motion.' },
                  ],
                },
                {
                  id: 'how-it-works', title: 'How It Works',
            ...
            },
            {
              ...
              summary: 'Watch SwayForm throw a fist bump — then flip one switch so it nods along with it.',
              workspaceFile: 'swayform_ws/src/swayform_robot/swayform_robot/behaviors/fist_bump.py',
              relatedConcepts: ['Sequences', 'On/off switches'],
              steps: [
                {
                  id: 'what-it-does', title: 'What It Does',
                  blocks: [
                ...
                },
                {
                  ...
                  blocks: [
                    ...
                    { type: 'callout', tone: 'note', label: 'Look closely', text: 'Right now the head stays still through all of it. That\'s about to change — the nod is already written, it\'s just switched off.' },
                    { type: 'callout', tone: 'safety', label: 'Safety', text: 'The punch is quick and the torso base rotates. Keep hands and fingers away from both while it runs — hold your fist out at arm\'s length, not next to the robot\'s body.' },
                  ],
                },
                {
                  id: 'how-it-works', title: 'How It Works',
              ...
              completionSummary: { text: 'You saw two parts of the body move together as one gesture, and used an on/off switch to add a whole new movement without touching anything else.', conceptsUsed: ['Sequences', 'On/off switches'] },
            },
            {
              id: 'finger-count', title: 'Finger Count', kind: 'activity', difficulty: 'beginner', estimatedTime: '10–15 minutes',
              summary: 'Watch SwayForm hold up a fist — then pick a number and make it count on its fingers.',
              workspaceFile: 'swayform_ws/src/swayform_robot/swayform_robot/behaviors/finger_count.py',
              relatedConcepts: ['Sequences', 'Choosing a value'],
              steps: [
                {
                  id: 'what-it-does', title: 'What It Does',
                  blocks: [
                    { type: 'lead', text: 'Finger Count is SwayForm answering a question with its hand. It lifts its arm the way it does for a wave — but instead of waving, it makes a fist and holds it up. Give it a number from 1 to 5 and it opens that many fingers, one at a time, holds them up for a few seconds so you can see, then relaxes.' },
                    { type: 'p', text: 'It counts the way a person does: index finger first, then middle, ring, pinky — and the thumb only comes out for five. Someone had to decide that. Robots don\'t know how humans count until you tell them.' },
                  ],
                },
                {
                  id: 'what-to-watch', title: 'What to Watch',
                  blocks: [
                    ...
                    { type: 'callout', tone: 'note', label: 'Look closely', text: 'Try 4 and then 5. The only difference is the thumb — and that\'s exactly how you\'d do it too.' },
                    { type: 'callout', tone: 'safety', label: 'Safety', text: 'Keep your fingers away from its fingers while they open and close.' },
                  ],
                },
                {
                  id: 'how-it-works', title: 'How It Works',
                  blocks: [
                    { type: 'p', text: 'The steps will feel familiar by now — the new thing is that the recipe has a blank in it. One step says "show the number," and the robot fills in which fingers to open based on a number you choose. No number, and it just keeps the fist.' },
                    { type: 'code', lang: 'text', filename: 'The recipe for a finger count', code: 'make a fist\n  ↓\nlift arm into position\n  ↓\nshow the number        ← fill in the blank: 1, 2, 3, 4, or 5\n  ↓\nhold for four seconds\n  ↓\nopen hand and go home' },
                    { type: 'p', text: 'That blank is the difference between a gesture that always does one thing and a gesture that can do several. Wave had a number you could change, too — but here the number *is* the point of the gesture.' },
                  ],
                },
                {
                  id: 'look-at-this-part', title: 'Look at This Part',
                  blocks: [
                    { type: 'p', text: 'Open the file. The blank is the very first line after the imports — and it\'s empty right now:' },
                    { type: 'code', lang: 'python', filename: 'finger_count.py', code: 'NUMBER = None   # the blank — nothing chosen yet\n\ndef close_fist(ctrl): ...         # make a fist\ndef raise_arm(ctrl): ...          # lift arm into position\ndef show_number(ctrl, number): ...  # open that many fingers\ndef open_and_return(ctrl): ...    # go home\ndef perform_finger_count(mock=False): ...  # runs all of the above, in order', workspaceFile: 'swayform_ws/src/swayform_robot/swayform_robot/behaviors/finger_count.py', line: 12 },
                    { type: 'p', text: '`None` is how the file says "no number yet." Everything below it already knows what to do with whatever you put there.' },
                  ],
                },
                {
                  id: 'try-it', title: 'Try It',
                  blocks: [
                    { type: 'p', text: 'Pick a number. Any whole number from 1 to 5 — there\'s no right answer, just yours.' },
                    { type: 'p', text: 'Find `NUMBER = None` at the top of the file and replace `None` with your number, so it reads something like `NUMBER = 3`. Then click **Run**.' },
                    { type: 'p', text: 'When Run says you\'re done, **Queue on Robot** lights up. Send it and watch SwayForm hold up your number. Then come back, change it, and do it again — this is the first gesture where you get to decide what the robot says.' },
                  ],
                },
              ],
            ...
            },
            {
              ...
              summary: 'The final challenge: put five steps in the right order, then aim SwayForm through its own camera and make it shake a real hand.',
              workspaceFile: 'swayform_ws/src/swayform_robot/swayform_robot/behaviors/target_lock.py',
              relatedConcepts: ['Sequencing', 'Live control', 'Robot vision'],
              steps: [
                {
                  id: 'what-it-does', title: 'The Final Challenge',
                  blocks: [
                    { type: 'lead', text: 'Every gesture so far has been a recording: you press go, SwayForm plays it back. This one is different. A window opens showing exactly what the robot\'s camera sees, with a red crosshair in the middle. You turn its head with your arrow keys until the crosshair sits on a person\'s outstretched hand, press Enter — and SwayForm works out where that hand is in the room, reaches for it, and shakes it.' },
                    { type: 'p', text: 'The hard robotics is already written: measuring distance with the depth camera, turning a point in a picture into a point in the room, working out the angle of every joint in the arm so the palm arrives there. Each of those jobs is packed inside a function you can use without reading it.' },
                    { type: 'p', text: 'What is missing is the plan. The program has five empty steps, and five functions waiting to fill them. Put them in an order that makes sense and the robot works. Put them in the wrong order and it refuses to move — which is exactly what a safe robot should do.' },
                  ],
                },
                {
                  id: 'the-five-functions', title: 'The Five Functions',
                  blocks: [
                    { type: 'p', text: 'Here they are, in alphabetical order — which is not the order they run in. Read what each one does, and notice what each one needs before it can work.' },
                    { type: 'heading', level: 3, text: 'activate_crosshair' },
                    { type: 'p', text: 'Brings up the robot\'s camera view with the targeting crosshair in the center. This is the robot\'s eyes — nothing can be aimed, and nothing can be drawn on the view, until the view exists.' },
                    { type: 'heading', level: 3, text: 'load_controls' },
                    { type: 'p', text: 'Draws the arrow-key controls in the corner of the camera view and gets the program ready to listen to your keyboard. The keys do not move anything yet — the controls are loaded, not unlocked.' },
                    { type: 'heading', level: 3, text: 'lock_and_shake' },
                    { type: 'p', text: 'Waits for you to press Enter. Then it freezes the target, measures how far away it is, works out how the arm has to bend to reach it, reaches, grips, shakes, and lets go. It can only lock onto something you have already aimed at.' },
                    { type: 'heading', level: 3, text: 'run_system_check' },
                    { type: 'p', text: 'Confirms that everything the robot is about to rely on is really there: camera available, robot connected, motion system ready, target system ready. It checks the pieces that have been set up — so anything it is supposed to check has to be set up first. If any check fails, the program stops before a single motor moves.' },
                    { type: 'heading', level: 3, text: 'unlock_movement' },
                    { type: 'p', text: 'Hands you the controls. From this moment the arrow keys really turn the robot\'s head, Esc brings it back to center so you can re-aim, and it keeps listening until you commit to a target. A robot should never hand over control before it knows it is safe to.' },
                  ],
                },
                {
                  id: 'thinking-about-order', title: 'Thinking About the Order',
                  blocks: [
                    { type: 'p', text: 'Nobody is going to give you the order — but every clue you need is in the descriptions. Ask the same question about each function: what has to be true before this can work?' },
                    { type: 'list', items: [
                      'Before you can aim, the robot needs a target display.',
                      'Before controls can appear on the camera view, there has to be a camera view.',
                      'A check can only confirm things that already exist.',
                      'Before movement is unlocked, the system has to confirm everything is ready.',
                      'You cannot lock onto a target you have not aimed at yet.',
                    ] },
                    { type: 'callout', tone: 'note', label: 'How engineers think', text: 'This is called a dependency: one step needing another to have happened first. Real robot software is full of them, and getting the order wrong is one of the most common ways robots fail.' },
                    { type: 'callout', tone: 'safety', label: 'Safety', text: 'During the live run, only the person holding out their hand stands near the robot, and they keep their hand still after Enter is pressed. Everyone else watches the screen.' },
                  ],
                },
                {
                  id: 'look-at-this-part', title: 'Look at This Part',
                  blocks: [
                    { type: 'p', text: 'Open the file. Near the top are five steps, and every one of them is an empty pair of parentheses:' },
                    { type: 'code', lang: 'python', filename: 'target_lock.py', code: '# Put one function name inside each pair of parentheses.\nSTEP_1 = ()\nSTEP_2 = ()\nSTEP_3 = ()\nSTEP_4 = ()\nSTEP_5 = ()', workspaceFile: 'swayform_ws/src/swayform_robot/swayform_robot/behaviors/target_lock.py', line: 19 },
                    { type: 'p', text: 'Just below, `perform_target_lock` runs whatever you put there, one step after another, top to bottom. If a step is empty, or a function finds that something it needs has not happened yet, the program stops right there.' },
                    { type: 'p', text: 'Write only the name — no quotes, no extra parentheses. A filled-in step looks like `STEP_1 = (some_function)`.' },
                  ],
                },
                {
                  id: 'try-it', title: 'Try It',
                  blocks: [
                    { type: 'p', text: 'Fill in all five steps, using each function exactly once, then click **Run**. If the order is not right yet, Run tells you how many steps are in the right place — not which ones. Go back to the descriptions and think it through.' },
                    { type: 'p', text: 'When Run says you\'re done, **Queue on Robot** lights up. Send it. Once it is approved and starts, the live view opens on this page by itself: the crosshair appears, the controls load, the checks tick green, and movement unlocks. Aim with the arrow keys, press Esc to re-center if you need to, and press **Enter** when the hand is inside the crosshair.' },
                  ],
                },
              ],
              completionSummary: { text: 'You assembled a robot\'s plan in the right order, aimed a humanoid through its own camera, picked a real point in the room, and made it reach out and touch it. You just programmed a humanoid robot.', conceptsUsed: ['Sequencing', 'Dependencies', 'Live control', 'Robot vision'] },
            },
          ],
        },
      ],
